...
@section('content')
    ...
    <div class="container py-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 border-3 p-2">
            <div class="col-span-1 lg:col-span-2"><a href="{{route('subject.admin' , ['cycleid' => session('cycleid')])}}" class="text-lg text-blue-400 cursor-pointer">Mis Materias</a>
                <a href="{{route('allsubject.index')}}" class="text-lg text-blue-400 cursor-pointer mx-6">Todas las Materias</a></div>
            @foreach ($subjects as $subject)
                @foreach ($subject->subjects as $item)
                <a href="{{ route('subject.index' , ['subjectId' => $item->subjectid , 'cycleid' => $cycles->cycledetailid]) }}" class="block bg-white h-60 text-center rounded-md border-2 cursor-pointer">
    ...
    <script>
        ...
        // Cuando se cambia la selección en el select
            ...
            window.location.href = '/Materias/' + selectedCicleId;
        };
